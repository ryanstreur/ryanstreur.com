defmodule RsdcWeb.UsersAuthTest do
  use RsdcWeb.ConnCase, async: true

  alias Rsdc.Accounts
  alias RsdcWeb.UsersAuth
  import Rsdc.AccountsFixtures

  @remember_me_cookie "_rsdc_web_users_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RsdcWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_users/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RsdcWeb.Endpoint.subscribe(live_socket_id)
